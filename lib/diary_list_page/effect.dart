...
void _onLoadMore(Action action, Context<DiaryListState> ctx) async {
  ctx.state.page++;
  List<Diary> data = await  BombApi(ctx.context).getPageData(page: ctx.state.page,count: ctx.state.count);
  ctx.dispatch(DiaryListActionCreator.loadMore(data.map((e) => DiaryItemState(diary: e)).toList()));
